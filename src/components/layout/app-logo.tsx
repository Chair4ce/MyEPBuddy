...
interface AppLogoProps {
  /** Show icon stacked above text when there's room */
...
/**
 * The myEPBuddy SVG logo icon.
 * White paths on transparent background - use className for color overrides.
 */
...
/**
 * Full app logo with icon and text.
 * Use variant="stacked" for vertical layout with icon above text.
 */
export function AppLogo({
  ...
  if (iconOnly) {
    return <AppLogoIcon size={config.icon} className="text-primary" />;
  }
  ...
  if (variant === "stacked") {
    return (
      ...
      >
        <AppLogoIcon size={config.icon} className="text-primary" />
        <span
          className={cn(
