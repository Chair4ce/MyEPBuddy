import { ImageResponse } from "next/og";

export const runtime = "edge";

export const alt = "My EPBuddy - Air Force EPB Statement Generator";
export const size = {
  width: 1200,
  height: 600,
};
export const contentType = "image/png";

export default async function Image() {
  return new ImageResponse(
    (
      <div
        style={{
          height: "100%",
          width: "100%",
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          justifyContent: "center",
          background: "linear-gradient(135deg, #082429 0%, #0d4b3b 50%, #082429 100%)",
          fontFamily: "system-ui, sans-serif",
        }}
      >
        {/* Background pattern */}
        <div
          style={{
            position: "absolute",
            top: 0,
            left: 0,
            right: 0,
            bottom: 0,
            backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.03'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
            opacity: 0.5,
          }}
        />

        {/* Main content */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
            justifyContent: "center",
            textAlign: "center",
            padding: "0 60px",
          }}
        >
          {/* Logo/Title */}
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              marginBottom: 24,
            }}
          >
            <svg
              width="80"
              height="72"
              viewBox="0 0 29 26"
              fill="none"
              style={{ marginBottom: 16 }}
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M9.50422 4.79102L21.8489 4.79074C21.8555 4.79102 22.604 4.75032 23.0328 5.01185C23.3211 5.18768 23.6069 5.41869 23.7957 5.80477C23.8893 5.99609 23.9075 6.11852 23.9582 6.32539C24.0229 6.58986 24.0415 6.74283 24.0696 7.01367C24.1014 7.32043 24.1 7.44232 24.1 7.80281C24.1 7.80281 25.5938 7.81212 26.071 7.91171C26.7331 8.04991 27.0619 8.24822 27.5774 8.73562C28.0773 9.20837 28.3126 9.67163 28.5627 10.3778C28.8831 11.2824 28.8243 11.9163 28.7168 12.8921C28.631 13.6709 28.4942 14.0985 28.2302 14.816C28.0095 15.416 27.8636 15.7496 27.5384 16.2752C27.3068 16.6496 26.88 17.1662 26.8725 17.1753C26.8723 17.1662 26.8606 16.5932 26.8157 16.2695C26.7638 15.8956 26.6697 15.5497 26.4198 15.1383C26.2058 14.786 26.0122 14.518 25.6692 14.3819C24.9691 14.1043 23.6362 14.1636 22.3399 14.2879C21.1042 14.4064 19.2208 14.9627 19.1905 14.9716C18.7542 15.15 18.4008 15.2772 18.0535 15.4108C17.8617 15.4846 17.6717 15.5606 17.4705 15.6483L24.8098 25.6578L13.9526 25.6125L2.99361 10.624V7.47097L14.9069 23.7636H21.1347L5.99361 3.1634V0L7.6659 2.29448L9.50422 4.79102ZM21.1871 6.5296H10.6897L16.4159 14.2299C16.5741 14.1627 16.7608 14.0844 16.9875 13.9909C17.0901 13.9486 17.2012 13.9034 17.3212 13.8541C18.0873 13.5392 19.3055 13.1442 19.3178 13.1402C19.3253 13.1382 20.5933 12.8018 21.4179 12.6743C22.9659 12.4349 23.4726 12.3729 24.9789 12.441C25.5745 12.4679 25.9404 12.5628 26.4377 12.8921C26.708 13.0711 27.1803 13.6406 27.1803 13.6406C27.1803 13.6406 27.3549 12.679 27.3844 12.1473C27.4149 11.5947 27.4152 11.2252 27.1803 10.75C26.95 10.2841 26.7405 10.1119 26.3366 9.87672C25.8309 9.58238 25.3916 9.48545 24.7879 9.48545H20.4007L19.3086 7.80281H22.5365C22.5365 7.66326 22.5344 7.47191 22.5005 7.30144C22.4521 7.05851 22.3687 6.79515 22.1349 6.65338C21.8909 6.5054 21.1871 6.5296 21.1871 6.5296Z"
                fill="#33c27f"
              />
              <path
                d="M6.4116 23.7636H10.0106L11.1847 25.6125H5.4372L0 18.2087L0.00196727 14.9716L6.4116 23.7636Z"
                fill="#33c27f"
              />
            </svg>
            <span
              style={{
                fontSize: 64,
                fontWeight: "bold",
                color: "white",
                letterSpacing: "-0.02em",
              }}
            >
              myEPBuddy
            </span>
          </div>

          {/* Tagline */}
          <div
            style={{
              fontSize: 28,
              color: "#afacab",
              marginBottom: 32,
              maxWidth: 800,
              lineHeight: 1.4,
            }}
          >
            AI-Powered EPB Statement Generator for Air Force Enlisted
          </div>

          {/* Features */}
          <div
            style={{
              display: "flex",
              gap: 24,
            }}
          >
            {["Track Accomplishments", "Generate Statements", "myEval Ready"].map(
              (feature) => (
                <div
                  key={feature}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                    padding: "10px 20px",
                    background: "rgba(255, 255, 255, 0.1)",
                    borderRadius: 24,
                    border: "1px solid rgba(255, 255, 255, 0.2)",
                  }}
                >
                  <span style={{ color: "#33c27f", fontSize: 18 }}>✓</span>
                  <span style={{ color: "#f7f5f4", fontSize: 18 }}>{feature}</span>
                </div>
              )
            )}
          </div>
        </div>
      </div>
    ),
    {
      ...size,
    }
  );
}


