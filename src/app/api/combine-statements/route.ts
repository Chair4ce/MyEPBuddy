...
export async function POST(request: Request) {
  ...
  try {
    ...
    const usageCheck = await checkAndTrackUsage(user.id, "combine_statements", modelId, apiKeys, billableCtx.idempotencyKey);
...
Output ONLY the combined statement.`;
    ...
    return cacheBillableJson(billableCtx, { combined }, usageCheck);
  } catch (error) {
    if (billableCtx) {
      return handleBillableLLMError(error, "POST /api/combine-statements", modelId, billableCtx);
    }
