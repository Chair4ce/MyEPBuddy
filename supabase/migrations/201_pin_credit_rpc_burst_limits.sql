-- Pin burst/daily abuse caps inside credit RPCs. Callers (including authenticated
-- PostgREST) previously could pass p_burst_limit / p_daily_limit and raise caps.
-- Drop those parameters; limits are SQL constants matching prior defaults.

-- ---------------------------------------------------------------------------
-- consume_credit: burst cap = 5 / 60s (excludes style_signature_llm)
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS consume_credit(UUID, TEXT, TEXT, TEXT, INT, TEXT);
...
BEGIN
  ...
  IF v_burst_count >= c_burst_limit THEN
    RETURN -1;
  END IF;
...
-- ---------------------------------------------------------------------------
-- check_and_record_usage (BYOK): burst cap = 5 / 60s
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS check_and_record_usage(UUID, TEXT, TEXT, TEXT, INT);
...
REVOKE ALL ON FUNCTION check_and_record_usage(UUID, TEXT, TEXT, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION check_and_record_usage(UUID, TEXT, TEXT, TEXT) TO authenticated, service_role;

-- ---------------------------------------------------------------------------
-- try_record_style_signature_app_call: daily cap = 12
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS try_record_style_signature_app_call(UUID, INT);
...
REVOKE ALL ON FUNCTION try_record_style_signature_app_call(UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION try_record_style_signature_app_call(UUID) TO authenticated, service_role;
