-- issue_managed_member_invite_token uses gen_random_bytes(), which lives in
-- the extensions schema. Migration 195 set search_path = public only, so the
-- RPC raises "function gen_random_bytes(integer) does not exist" in production
-- and /api/team/invite-managed-member returns 500 "Failed to create invite link".
